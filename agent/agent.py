...
try:
    ...
    print("ERROR: pynput not installed. Run:  pip install pynput requests python-dotenv")
    sys.exit(1)

try:
    from dotenv import load_dotenv
except ImportError:
    def load_dotenv(*a, **k): pass  # graceful degradation
...
class AgentState:
    def __init__(self):
        ...
        self.status          = "active"   # active | idle | away
    ...
    def total_seconds(self):
        return self.active_seconds + self.idle_seconds + self.away_seconds
...
def monitor_loop():
    """Runs every second — checks idle/away thresholds and sends heartbeats."""
    heartbeat_counter = 0
